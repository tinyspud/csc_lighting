...
uint32_t Fapi_BlockErase( uint32_t Bank, uint32_t Erase_Start_Address, uint32_t Size_In_Bytes)
{
    ...
    for (bk = ucStartBank; bk < (ucEndBank + 1); bk++){
    	...
    	//}
    	//else if(Bank == 1){
    	//	status= Fapi_enableMainBankSectors(0xFFFD);        /* used for API 2.01*/
    	//}
...
uint32_t Fapi_BlockProgram( uint32_t Bank, uint32_t Flash_Start_Address, uint32_t Data_Start_Address, uint32_t Size_In_Bytes)
{
	...
	if (Size_In_Bytes < 16)
		...
		bytes = 16;
	/* bug fix to accomodate last sector on bank 1*/
	ucBank = 1;
	for (i = 0; i < NUMBEROFSECTORS-1; i++){
...
{
	uint32_t error=0;
	volatile uint32_t * src1 = (uint32_t *) Source_Start_Address;
	volatile uint32_t * dst1 = (uint32_t *) Program_Start_Address;
	uint32_t bytes = No_Of_Bytes;
